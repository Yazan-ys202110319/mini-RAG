...
import re # Regix is a library to edit text
...
class DataController(BaseController):
    ...
    # Constructor 
    def __init__(self):
        super().__init__() # To let the child call the parent (BaseController)
        self.size_scale = 1048576 # Convert MB to Bytes


    def validate_uploaded_file(self, file: UploadFile):
                                    # DataController got the app_settings from the parent
        if file.content_type not in self.app_settings.FILE_ALLOWED_TYPES: 
            return False, ResponseSignal.FILE_TYPE_NOT_SUPPORTED.value
        
        # file.size will return the value in Bytes
    ...
    def generate_unique_filepath(self, orig_file_name: str, project_id: str):
        ...
        # Check if the file name is already exists, if yes create a new one, because its not 
        # unique any more
